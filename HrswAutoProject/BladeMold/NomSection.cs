...
namespace Gy.HrswAuto.BladeMold
{
    public class NomSection
    {
        ...
        #endregion
        public void ReadSection1(StreamReader reader)
        {
            string line = "";
            ...
            while(!reader.EndOfStream)
            {
                ...
                if (line.Contains("SECTION"))
                {
                    ...
                    _name = strs[1];
                    _pointCount = int.Parse(strs[2]);
                    _sectionZd = double.Parse(strs[3]);
                }
            ...
            for (int i = 1; i < _pointCount; i++)
            {
                line = reader.ReadLine();
                _sectionStrings.Add(line); // 添加原始字符串
                var ps = line.Split(" ".ToCharArray(), StringSplitOptions.RemoveEmptyEntries);
                PointF np = new PointF();
                newP.X = float.Parse(ps[0]);
                newP.Y = float.Parse(ps[1]);
                _sectionPoints.Add(np);
            }
            FindNoseTail();
        }
        /// <summary>
